...
const FeedbackItem = ({
  ...
  return (
    ...
    onClick={()=>handleView && handleView(suggestions)}
    >
      <div className="flex items-start gap-6">
        <button className={`flex flex-col 
        items-center gap-1 rounded-lg px-3 py-2
         transition-all ${suggestions.upvoted ? 'bg-blue-600 text-white':'bg-gray-100 hover:bg-blue-100 text-gray-800'}` } 
        onClick={(e)=>{
          e.stopPropagation()
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-gray-800">{suggestions.comment}</span>
        </div>
      </div>
    </div>
  )
